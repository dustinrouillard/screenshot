...
curl $CLIPBOARD -s -L > ${MAIN_FOLDER}/${FOLDER_NAME}/${FILE_NAME}.png

# Send Uploading Notification
osascript -e 'display notification "'"$FILE_NAME"'" with title "Uploading Link from clipboard"'
...
osascript -e 'display notification "'"$FILE_URL"'" with title "Uploaded link from clipboard"'
